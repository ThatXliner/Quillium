...
That's the gap. The collaboration infrastructure is solid, but it's built on top of a writing model that doesn't support how writers actually revise. Quillium already has branching, which means you can fork a sentence, keep three versions alive, and decide later. The question for us was what collaboration should look like when the document itself supports parallel exploration.

## What we're actually building

[Quillium Omni](/omni) is designed around a different principle: shared text, independent views.

You and your collaborator see the same document, the same text, the same annotations. Your UI state, though, is yours. Nobody else's modals pop up on your screen, and nobody else's scroll position drags you around. You're working in the same material, but each of you is in your own headspace, which should feel intuitive once you're in it.
...
If you want to see what someone else is doing, you can opt into follow mode (the way Figma does it). Click on a collaborator's avatar, and your view mirrors theirs, including their scroll position, their cursor, and even which revision version they're looking at. Click away and you're back to your own view.

The key design decision is that collaboration is additive. Nothing about the core writing experience changes. You still branch text, you still have your revision history, you still write offline. Omni layers on top; it doesn't replace what's underneath.
...
This matters because it means collaboration can happen around the writing without disrupting the writing itself. Your collaborator can leave detailed feedback on a draft while you're actively revising a different section, and the annotations don't conflict with your edits, because they exist alongside the text rather than inside it.

Because Quillium already has branching, your collaborator's suggestion can literally become a revision. "Try it this way" turns into a new version of the sentence that lives next to your original. You compare, you pick, or you keep both. That's what a real writing conversation looks like.

## What we're not building

We're not building a web client (yet). Collaboration in v1 is desktop-to-desktop, so both people need the Quillium app. This is partly a resource constraint and partly intentional, because writing apps belong on your desktop, and also because we'd rather get the core experience right than spread thin across platforms.

We're not building Google Docs-style simultaneous typing as the primary interaction. You *can* edit at the same time, and those edits will sync correctly. Quillium isn't optimized for two people typing into the same paragraph at once, though; it's optimized for two people thinking about the same paragraph in their own time and their own way.
