...
function resolveDependencyPackageJson(name: string): string | null {
    const candidates = [
...
const jsDeps: LicenseEntry[] = Object.keys(pkgJson.dependencies ?? {}).flatMap((name) => {
    try {
        const packageJsonPath = resolveDependencyPackageJson(name);
        if (!packageJsonPath) return [];
        const depPkg = JSON.parse(readFileSync(packageJsonPath, "utf-8"));
...
const cargoToml = readFileSync(join(packageRoot, "src-tauri/Cargo.toml"), "utf-8");
...
writeFileSync(join(packageRoot, "static/licenses.json"), `${JSON.stringify(all, null, 2)}\n`);
