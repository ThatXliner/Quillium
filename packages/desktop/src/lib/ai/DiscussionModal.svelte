...
<dialog aria-label={title} use:show onclose={onclose} class="discussion-modal rounded-2xl bg-gray-100 shadow-xl">
    ...
    {#if error}<p role="alert" class="px-5 py-2 text-sm text-red-700">{error}</p>{/if}
    <div class="flex min-h-0 flex-1 flex-col">{@render children()}</div>
</dialog>
<style>
.discussion-modal { margin: auto; position: fixed; inset: 0; width: min(760px, calc(100vw - 32px)); height: min(760px, calc(100dvh - 48px)); max-height: calc(100dvh - 48px); padding: 0; color: #27272a; }
