...
export function getPrimaryInlineAnnotation(
    ...
    return candidates.sort((a, b) => {
        const activeDelta =
            Number(b.id === activeAnnotationId) - Number(a.id === activeAnnotationId);
...
export function findAnnotationPath(
    ...
    for (const annotation of annotations) {
        ...
        for (const version of annotation.versions) {
            const nestedPath = findAnnotationPath(
                targetId,
                version.text,
                version.annotations ?? [],
            );
...
export type RevisionContextLayer = {
    annotation: RenderingRevisionAnnotation;
    before: string;
    revision: string;
    after: string;
    depth: number;
...
export function buildRevisionContextLayers(
    ...
    return path
        .filter(
            (entry): entry is AnnotationPathEntry & { annotation: RenderingRevisionAnnotation } =>
        ...
        if (includeNestedAnnotations) {
            ...
            nextAnnotations = [
                ...nextAnnotations.filter(
                    (annotation) =>
                        !mappedNestedAnnotations.some((nested) => nested.id === annotation.id),
                ),
