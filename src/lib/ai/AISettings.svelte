        ...
        <div class="flex flex-col gap-1">
            {#each PROVIDERS as provider}
                <button
                    ...
                    class="flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-left transition-colors
                ...
                >
                    ...
                    <span class="text-sm font-medium text-black/75">{provider.label}</span>
                    {#if selectedProvider === provider.id}
                        <div class="ml-auto w-1.5 h-1.5 rounded-full bg-blue-500 shrink-0"></div>
                    {/if}
                </button>
            {/each}
        </div>
