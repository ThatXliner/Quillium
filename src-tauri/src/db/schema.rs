...
pub fn open_db(path: &Path) -> Result<Connection> {
    ...
    // Migration: add draft branching columns to documents if they don't exist yet.
    let doc_columns: Vec<String> = {
    ...
    if !doc_columns.iter().any(|c| c == "parent_document_id") {
        conn.execute(
            "ALTER TABLE documents ADD COLUMN parent_document_id TEXT DEFAULT NULL REFERENCES documents(id) ON DELETE SET NULL",
            [],
        )?;
    }
    if !doc_columns.iter().any(|c| c == "branched_from_snapshot_id") {
        conn.execute(
            "ALTER TABLE documents ADD COLUMN branched_from_snapshot_id INTEGER DEFAULT NULL",
            [],
        )?;
    }
...
pub fn init_schema(conn: &Connection) -> Result<()> {
    conn.execute_batch(
        ...
        CREATE TABLE IF NOT EXISTS documents (
            ...
            parent_document_id       TEXT DEFAULT NULL REFERENCES documents(id) ON DELETE SET NULL,
            branched_from_snapshot_id INTEGER DEFAULT NULL
        );
